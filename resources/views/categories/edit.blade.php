    ...
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST">
            ...
            {{-- <div class="mb-4">
                ...
                <select name="tenant_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($tenants as $tenant)
                        <option value="{{ $tenant->id }}" {{ $category->tenant_id == $tenant->id ? 'selected' : '' }}>{{ $tenant->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="business_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($businesses as $business)
                        <option value="{{ $business->id }}" {{ $category->business_id == $business->id ? 'selected' : '' }}>{{ $business->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" class="mt-1 block w-full border rounded p-2" value="{{ $category->name }}">
                @error('name') <span class="text-red-500">{{ $message }}</span> @enderror
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium">Description</label>
                <textarea name="description" class="mt-1 block w-full border rounded p-2">{{ $category->description }}</textarea>
