          ...
          <div class="mb-3">
              ...
              <select name="tenant_id" id="tenant_id" class="form-control">
                  @foreach ($tenants as $tenant)
                      <option value="{{ $tenant->id }}" {{ old('tenant_id', $employee->tenant_id) == $tenant->id ? 'selected' : '' }}>{{ $tenant->name }}</option>
                  @endforeach
              </select>
              @error('tenant_id')
          ...
          <div class="mb-3">
              ...
              <select name="spatie_role" id="spatie_role" class="form-control">
                  @foreach ($spatieRoles as $spatieRole)
                      <option value="{{ $spatieRole->name }}" {{ old('spatie_role', $employee->roles->first()->name ?? '') == $spatieRole->name ? 'selected' : '' }}>{{ ucfirst($spatieRole->name) }}</option>
                  @endforeach
              </select>
              @error('spatie_role')
                  <div class="text-danger">{{ $message }}</div>
