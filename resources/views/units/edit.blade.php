        ...
        <form action="{{ route('units.update', $unit) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="tenant_id" class="mt-1 block w-full border rounded p-2">
                    <option value="" {{ is_null($unit->tenant_id) ? 'selected' : '' }}>None</option>
                    @foreach ($tenants as $tenant)
                        <option value="{{ $tenant->id }}" {{ $unit->tenant_id == $tenant->id ? 'selected' : '' }}>{{ $tenant->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="business_id" class="mt-1 block w-full border rounded p-2">
                    @foreach ($businesses as $business)
                        <option value="{{ $business->id }}" {{ $unit->business_id == $business->id ? 'selected' : '' }}>{{ $business->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" class="mt-1 block w-full border rounded p-2" value="{{ $unit->name }}">
                @error('name') <span class="text-red-500">{{ $message }}</span> @enderror
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium">Short Name</label>
                <input type="text" name="short_name" class="mt-1 block w-full border rounded p-2" value="{{ $unit->short_name }}">
